...
function RenderIssue ({issue,i,returnBook}) {
    ...
    let fine=0;
    //   if(((returnDate.getTime()-today.getTime())/(1000 * 60 * 60 * 24))>0)
    //   {
    // }
    fine=Math.floor((returnDate.getTime()-today.getTime())/(1000 * 60 * 60 * 24))*-fineRate;
    // console.log(Math.floor((returnDate.getTime()-today.getTime())/(1000 * 60 * 60 * 24))*fineRate);
    totalFine+=fine;
    console.log("total "  +totalFine);
    return (
            <React.Fragment>
            <td>
            {i}
